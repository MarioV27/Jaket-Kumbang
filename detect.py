        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if len(det):
                    ...
                    jumlahmanusia = 0
                    for manusia in det:
                        if manusia[5] == 7 and manusia[3]>850:
                            area.append(manusia[0])
                            area.append(manusia[1])
                            area.append(manusia[2])
                            area.append(manusia[3])
                            jumlahmanusia +=1
                    
                    orangke={}

                    for t in range(jumlahmanusia):
                        orangke[f'orangke_{t}'] = [] 
                    for deteksi in det:
                        center_x = (int(deteksi[0])+int(deteksi[2])) / 2
                        center_y = (int(deteksi[1])+int(deteksi[3])) / 2
                        for x in range(jumlahmanusia):
                            x1 = int(area[(x*4)+0])
                            y1 = int(area[(x*4)+1])
                            x2 = int(area[(x*4)+2])
                            y2 = int(area[(x*4)+3])
                            if x1 < center_x < x2 and y1 < center_y < y2 and deteksi[5] != 7:
                                deteksilist = deteksi.tolist()
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            ...
                            # cv2.line(im0,[0,270],[1080,270],(0,255,0),2)
                        if save_crop:
